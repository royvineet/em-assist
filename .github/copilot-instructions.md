...
This rebuilds the index and checks the data directory is accessible.
Data sync (git remote, database, etc.) can be configured in `scripts/startup.sh` when ready.
...
├── profiles/
│   ├── directives.md     # hard rules and constraints — READ THIS FIRST
...
│   ├── calendar.md       # public holidays, company holidays, team member leave
...
└── files/
    ...
    │   └── <subcategory>/
    ├── reports/          # status reports, summaries, perf reviews
    │   └── <subcategory>/
...
**Always read `directives.md` first** for any scheduling, priority, or suggestion decision.
**Then read `index.yaml`.** It is a compact summary of all meetings, projects, and files.
Use it to decide which specific files to open.

### Search hierarchy

```
1. Read profiles/directives.md                  (always, for any scheduling/priority decision)
         ↓
2. Read index.yaml                              (always, instant)
         ↓
3. Identify relevant subset:
   ...
   - File / doc?       → index.yaml files[]    → open the path
         ↓
4. Read only the identified files
         ↓
5. Answer grouped by source
```
...
| "who is on leave?" | profiles/calendar.md + profiles/team/\<name>.md availability |
...
meetings:
  - date: "2026-03-14"
    type: standup
    qualifier: ""              # e.g. "priya-sharma" for 1:1
...
projects:
  - key: platform-v2
    ...
    status: active
    summary: "Event-driven pipeline rewrite; M2 at-risk."
    tags: [kafka, go, react]
...
Tracks both things the manager does directly and work delegated to team members
that the manager is accountable for.
...
actions:
  - id: 1
    ...
    source_meeting: "2026-03-10-planning"  # meeting filename stem or null
    tags: [okr, planning]
...
1. Run the scaffolding script:
   ...
   python scripts/new-meeting.py <type> [qualifier]
   ```
2. Pre-fill: date, time, attendees (from events.yaml), project.
...
## Work items  (table: ticket, title, owner, estimate)
## Risks flagged
## Action items
```
...
### `directives.md` ← READ FIRST

Hard rules and constraints that override default behavior. Read this before making
any scheduling, prioritization, or suggestion decision.

```markdown
# Directives

## Working hours & protected time
- [e.g. No meetings before 9am or after 6pm]
- [e.g. Tuesday mornings are deep-work blocks — no interruptions]

## Communication preferences
- [e.g. Async-first; synchronous only for unblocking]

## Team norms
- [e.g. On-call rotation: do not schedule planning on on-call handoff days]

## Current overrides
- [Temporary rules, e.g. "Freeze non-critical actions until Q2 OKRs are signed off"]
```
...
Public holidays, company-wide holidays, and individual team member leave periods.
Used for due-date checking and scheduling enrichment.
...
|----------------|------------|------------|---------------|--------------------|
| ravi-kumar     | 2026-04-07 | 2026-04-11 | planned leave |                    |
| priya-sharma   | 2026-06-01 | 2026-06-05 | planned leave | OOO, async only    |

## Blackout periods
| From       | To         | Reason                          |
|------------|------------|---------------------------------|
| 2026-03-28 | 2026-04-01 | Company all-hands / travel week |
```
...
| Project meeting (standup/retro/planning) | Flag at-risk milestones for that project |
| Action delegated to a team member | Check their profile and calendar.md for leave status |
| Action with a due date | Check calendar.md — flag if it falls on a holiday, leave day, or blackout period |
...
Project: platform-v2 (M2 at-risk).
```
...
```
1. Read profiles/directives.md
   → Apply any current overrides or hard constraints first

2. Read profiles/work-types.md
   → Filter: Duration ≤ available time
   → Filter: Energy matches current state
   → Filter: Context is compatible with location

3. Read profiles/goals.md (manager personal goals section)
   → Rank remaining work types by related goal priority

4. Check actions.yaml
...
- **Explain why you're asking.** "No project on file for that action — which one?"
...
- Milestone dates or interview round numbers the user didn't state
